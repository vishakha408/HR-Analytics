...
def __build_dept_promo_retrench_plots(df):
    ...
    # New section: Salary Analysis by Department
    with streamlit.expander(
        "Analysis: Department wise salary and compensation...", expanded=False
    ):
        utils.show_questions(
            [
                "* What is the average salary by department?",
                "* Are there significant compensation differences between departments?",
            ]
        )
        ...
        with streamlit.expander("View Insights...", expanded=False):
            utils.show_insights(
                [
                    ...
                    "* Departments with lower average salaries may need compensation reviews",
                    "* Competitive compensation is key to retention and attracting talent",
                ]
            )
