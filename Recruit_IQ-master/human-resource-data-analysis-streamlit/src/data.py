...
def load_transform(file) -> DataFrame:
    """Load the raw data and prepare/transform it (cached)"""
    ###
    ### Read raw data
    ###
    ...
    df["ToBePromoted"] = df["ToBePromoted"].fillna("No")
    ...
    df["ToBeRetrenched"] = df["ToBeRetrenched"].fillna("No")
    ...
    df["PctAtCompany"] = (df["YearsAtCompany"] / df["TotalWorkingYears"]).replace([np.inf, -np.inf], np.nan).fillna(0) * 100
